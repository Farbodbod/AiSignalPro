...
// (تمام کامپوننت‌های دیگر مانند Header, SystemStatus, و غیره کامل و بدون تغییر اینجا هستند)
...
// --- ✨ کامپوننت PriceTicker (نسخه عیب‌یاب) ✨ ---
const PriceTicker = () => {
    const [prices, setPrices] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null); // State جدید برای ثبت خطا
    ...
    const fetchPrices = useCallback(async () => {
        ...
        try {
            const response = await fetch('https://aisignalpro-production.up.railway.app/api/price-ticker/', {
                cache: 'no-cache', // جلوگیری از کش شدن پاسخ
            });
            if (response.ok) {
    ...
    return (
        <section className="bg-gray-800/30 backdrop-blur-lg rounded-xl p-4 border border-yellow-500/20">
            ...
            {/* --- بخش جدید برای نمایش وضعیت عیب‌یابی --- */}
            <div className="bg-red-900/50 border border-red-500/50 text-white p-2 rounded-md mb-4 text-xs font-mono break-all">
                <p><strong>Debug Info:</strong></p>
                <p>Loading: {loading.toString()}</p>
                <p>Error: {error ? error : 'None'}</p>
                <p>Prices Count: {prices.length}</p>
                <p>Raw Data: {JSON.stringify(prices)}</p>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {prices.map(p => (
                        <div key={p.symbol} className="flex justify-between items-center bg-black/20 p-2 rounded-md">
                            <div><p className="font-bold text-white">{p.symbol}/USDT</p><p className="text-xs text-gray-500 capitalize">{p.source}</p></div>
                            <div className="text-right">
                                <p className="font-semibold font-mono text-white">${p.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: p.price < 1 ? 4 : 2 })}</p>
