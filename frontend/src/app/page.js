...
const SystemStatus = () => {
    const services = [ { name: 'Kucoin', status: 'online' }, { name: 'OKX', status: 'online' }, { name: 'MEXC', status: 'offline' }, { name: 'Telegram', status: 'online' }, ];
    ...
    return (
        <div className="bg-black/30 rounded-lg p-4 border border-yellow-500/30 space-y-4 shadow-lg shadow-black/20">
            <div className="flex justify-between items-start">
                <div> <span className="font-bold text-lg text-white">{trade.symbol}</span> <span className={`text-xs ml-2 font-bold px-2 py-0.5 rounded-full ${isLong ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}> {isLong ? "LONG" : "SHORT"} </span> </div>
                <div className="text-right"> <span className={`font-mono text-xl font-bold ${isProfit ? 'text-green-400' : 'text-red-400'}`}>{pnlText}</span> <p className="text-xs text-gray-400">Current: {trade.current_price}</p> </div>
            </div>
            <div className="relative h-6 w-full bg-black/40 rounded-full overflow-hidden border-2 border-gray-700 flex items-center">
                ...
                <div className="absolute top-0 h-full flex justify-center items-center" style={{ left: `${entryPositionPercent}%`, transform: 'translateX(-50%)' }}>
                    ...
                    <div className="absolute top-1/2 -translate-y-1/2 text-xs font-bold text-black bg-yellow-400 px-1.5 py-0.5 rounded-md shadow-md"> {trade.entry_price} </div>
                </div>
            ...
            <div className="text-xs text-gray-300 bg-black/20 p-2 rounded-md">
                <p> <span className="font-bold text-blue-400">System: {trade.system_confidence}%</span> | <span className="font-bold text-yellow-500 ml-2">Live AI: {trade.ai_score}%</span> | <span className="ml-2">{trade.status_text}</span> </p>
            </div>
        </div>
    );
};
