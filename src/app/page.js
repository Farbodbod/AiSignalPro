...
const SystemStatus = () => {
  const services = [
    ...
    { name: 'MEXC', status: 'online', ping: '210ms' },
    { name: 'Telegram', status: 'online', ping: 'OK' },
  ];
  ...
  return (
    <section>
      <div className="grid grid-cols-3 md:grid-cols-5 gap-2 text-center">
        ...
        <div className="bg-yellow-500/80 text-black p-2 rounded-lg text-sm font-bold flex items-center justify-center cursor-pointer hover:bg-yellow-500 col-span-3 md:col-span-1">
          Test All
        </div>
      </div>
    </section>
...
const PriceTicker = () => {
  const prices = [
    ...
    { symbol: 'ETH/USDT', price: '3,540.12', change: '+3.5%', source: 'Gate.io' },
    { symbol: 'SOL/USDT', price: '165.80', change: '-1.2%', source: 'MEXC' },
    { symbol: 'XRP/USDT', price: '0.52', change: '+0.8%', source: 'Kucoin' },
    { symbol: 'DOGE/USDT', price: '0.15', change: '-3.4%', source: 'Gate.io' },
  ];
  return (
    <section className="bg-gray-800/30 backdrop-blur-lg rounded-xl p-4 border border-yellow-500/20">
        ...
        {prices.map(p => (
          <div key={p.symbol} className="flex justify-between items-center bg-black/20 p-2 rounded-md">
            ...
            <div className="text-right">
              ...
              <p className={'text-sm ' + (p.change.startsWith('+') ? 'text-green-400' : 'text-red-400')}>{p.change}</p>
            </div>
          </div>
        ))}
      </div>
    ...
    <div className={'rounded-xl p-4 border ' + color.border + ' ' + color.bg}>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-3 pt-3 text-sm">
        <div>
          ...
          <p className="font-mono text-white">{signal.entry}</p>
        </div>
        <div className="text-right">
          <p className="text-gray-400">Stop-Loss</p>
          <p className="font-mono text-red-400">{signal.sl}</p>
        </div>
        <div>
          <p className="text-gray-400">Targets</p>
          <div className="flex flex-wrap gap-x-3">{signal.targets.map(t => <span key={t} className="text-green-400 font-mono">{t}</span>)}</div>
        </div>
        <div className="text-right">
          <p className="text-gray-400">Accuracy</p>
          <p className="font-mono text-white">{signal.accuracy}</p>
        </div>
        <div>
          <p className="text-gray-400">Support</p>
          <div className="flex flex-wrap gap-x-3">{signal.support.map(s => <span key={s} className="text-white font-mono">{s}</span>)}</div>
        </div>
        <div className="text-right">
          <p className="text-gray-400">Resistance</p>
          <div className="flex flex-wrap gap-x-3 justify-end">{signal.resistance.map(r => <span key={r} className="text-white font-mono">{r}</span>)}</div>
        </div>
      </div>
...
const Signals = () => {
  const sampleSignals = [
    ...
    { type: 'sell', symbol: 'ETH/USDT', timeframe: '1h', entry: '3550-3560', sl: '3600', targets: ['3500', '3450'], confidence: 92, ai_score: 90, reasons: ['Bearish MACD Cross'], accuracy: "81%", support: ['3510', '3480'], resistance: ['3580', '3600'] },
  ];
  return (
    <section className="bg-gray-800/30 backdrop-blur-lg rounded-xl p-4 border border-yellow-500/20">
