...
    const progress = denominator !== 0 ? Math.min(Math.abs(trade.current_price - trade.entry_price) / denominator * 100, 100) : 0;
...
const AiChat = () => {
    const messages = [
        { sender: 'ai', text: 'Good morning! Bitcoin is showing strong bullish divergence on the 4h chart.' },
        { sender: 'ai', text: 'A new BUY signal for ETH/USDT has been generated. Confidence: 92%.' },
        { sender: 'user', text: 'Give me a detailed analysis for SOL/USDT.' },
    ];
    return (
...
            <div className="space-y-3 h-48 overflow-y-auto pr-2 text-sm">
                {messages.map((msg, i) => (
                    <div key={i} className={`p-2 rounded-lg ${msg.sender === 'ai' ? 'bg-black/20 text-gray-300' : 'bg-yellow-500/20 text-yellow-300 text-right'}`}>
                        {msg.text}
                    </div>
                ))}
            </div>
             <div className="mt-3 flex gap-2">
                <input type="text" placeholder="Ask AI..." className="flex-grow bg-black/30 border border-gray-700 rounded-lg p-2 text-sm focus:ring-yellow-500 focus:border-yellow-500"/>
                <button className="bg-yellow-500 text-black font-bold p-2 rounded-lg">Send</button>
            </div>
        </section>
    );
};
...
                <select className="flex-grow bg-black/30 border border-gray-700 rounded-lg p-2 text-sm">
...
                </select>
            </div>
            <div className="text-sm text-gray-400">Analysis details for the selected pair will be shown here...</div>
        </section>
    );
};
...
const SystemHealth = () => {
    const health = [
        { name: 'Analysis Engine', accuracy: '91.2%', speed: '1.2s' },
        { name: 'AI Engine', accuracy: '88.5%', speed: '3.5s' },
    ];
    return (
        <section className="bg-gray-800/30 backdrop-blur-lg rounded-xl p-4 border border-yellow-500/20">
...
                {health.map(h => (
                    <div key={h.name} className="flex justify-between items-center bg-black/20 p-2 rounded-md">
                        <span className="text-gray-300">{h.name}</span>
                        <div className="text-right">
                            <span className="font-mono text-white">{h.accuracy}</span>
                            <span className="text-xs text-gray-500 ml-2">({h.speed})</span>
                        </div>
                    </div>
                ))}
            </div>
