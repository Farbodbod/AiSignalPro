# engines/strategies/pivot_reversal.py (v2.1 - با تایید کندل استیک)
import logging
...
class PivotReversalStrategy(BaseStrategy):
    ...
    def _get_signal_config(self) -> Dict[str, Any]:
        return {"proximity_percent": self.config.get("proximity_percent", 0.002), "stoch_oversold": self.config.get("stoch_oversold", 25), "stoch_overbought": self.config.get("stoch_overbought", 75), "pivot_levels_to_check": self.config.get("pivot_levels_to_check", ['R2', 'R1', 'S1', 'S2'])}
    ...
    def check_signal(self) -> Optional[Dict[str, Any]]:
        ...
        for level_name in cfg['pivot_levels_to_check']:
            ...
            if abs(current_price - level_price) / current_price < cfg['proximity_percent']:
                if level_name.startswith('S') and stoch_data['position'] == "Oversold" and stoch_data['percent_k'] < cfg['stoch_oversold']:
                    signal_direction = "BUY"; trigger_level_name = level_name; break
                elif level_name.startswith('R') and stoch_data['position'] == "Overbought" and stoch_data['percent_k'] > cfg['stoch_overbought']:
                    signal_direction = "SELL"; trigger_level_name = level_name; break
        
        if not signal_direction: return None

        # --- ✨ ارتقای جدید: فراخوانی فیلتر تایید کندل استیک جهانی ---
        confirming_pattern = self._get_candlestick_confirmation(signal_direction)
        if not confirming_pattern:
            logger.info(f"[{self.strategy_name}] Pivot signal found but no candlestick confirmation.")
            return None # سیگنال بدون تایید کندلی رد می‌شود
            
        logger.info(f"✨ [{self.strategy_name}] Valid signal! {signal_direction} near {trigger_level_name} confirmed by {confirming_pattern} pattern.")
        ...
        if pivot_levels.get('P'): risk_params['targets'][0] = pivot_levels.get('P')
        
        return {
            "strategy_name": self.strategy_name, "direction": signal_direction, "entry_price": current_price,
            **risk_params, 
            "confirmations": {
                "trigger_pivot_level": trigger_level_name,
                "stochastic_k": stoch_data['percent_k'],
                "confirming_pattern": confirming_pattern # <-- افزودن نام الگو به دلایل سیگنال
            }
        }
