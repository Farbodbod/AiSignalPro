...
class BaseStrategy(ABC):
    ...
    def _log_criteria(self, criterion_name: str, status: bool, reason: str = ""):
        ...
        logger.info(
            f"  {status_emoji} Criterion: {self.name} on {self.primary_timeframe} - "
            f"'{criterion_name}': {status}. Reason: {reason}"
        )
    ...
    def _log_indicator_trace(self, indicator_name: str, value: Any, status: str = "OK", reason: str = ""):
        self.log_details["indicator_trace"].append(
            {"indicator": indicator_name, "value": str(value), "status": status, "reason": reason}
        )
        logger.debug(
            f"    [Trace] Indicator: {indicator_name} -> Value: {value}, Status: {status}, Reason: {reason}"
        )
    ...
    def _log_final_decision(self, signal: str, reason: str = ""):
        ...
        logger.info(
            f"{signal_emoji} Final Decision: {self.name} on {self.symbol} {self.primary_timeframe} -> "
    ...
    def get_indicator(self, name_or_alias: str, analysis_source: Optional[Dict] = None) -> Optional[Dict[str, Any]]:
        ...
        if name_or_alias in self.indicator_configs:
            ...
            if not global_indicator_config:
                ...
                self._log_indicator_trace(
                    name_or_alias, None, status="FAILED", reason="Not defined in global indicator config."
                )
                return None
        ...
        if not indicator_data or not isinstance(indicator_data, dict):
            self._log_indicator_trace(
                name_or_alias, None, status="FAILED", reason=f"Missing data object for key: {unique_key}."
            )
            return None
        ...
        if "error" in status or "failed" in status or status == "calculation incomplete - required columns missing":
            self._log_indicator_trace(
                name_or_alias, status, status="FAILED", reason=f"Indicator reported failure status: {status}"
            )
            return None
    ...
    def _get_trend_confirmation(self, direction: str) -> bool:
        ...
        target_htf = self.config.get(self.primary_timeframe)
        ...
        for rule_name, rule_params in htf_rules.items():
            ...
            if rule_name.lower() == "adx":
                ...
                if adx_strength >= rule_params.get("min_strength", 20) and direction.upper() in adx_dir.upper():
    ...
    def _calculate_smart_risk_management(self, entry_price: float, direction: str, stop_loss: float) -> Dict[str, Any]:
        if not isinstance(entry_price, (int, float)) or not isinstance(stop_loss, (int, float)):
            logger.debug(f"Risk calc skipped due to invalid inputs. Entry: {entry_price}, SL: {stop_loss}")
            return {}
        ...
        if direction.upper() == "BUY":
            targets = [r for r in sorted(resistances_prices) if r > entry_price][:3]
        elif direction.upper() == "SELL":
            targets = [s for s in sorted(supports_prices, reverse=True) if s < entry_price][:3]

        if not targets:
            logger.info(
                f"No key level targets found for {self.symbol}@{self.primary_timeframe}. Using reward ratios fallback."
            )
            reward_ratios = self.config.get("reward_tp_ratios", [1.5, 3.0, 5.0])
            targets = [
                entry_price + (total_risk_per_unit * r if direction.upper() == "BUY" else -total_risk_per_unit * r)
