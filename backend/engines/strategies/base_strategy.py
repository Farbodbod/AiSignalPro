...
class BaseStrategy(ABC):
    ...
    def get_indicator(self, name_or_alias: str, analysis_source: Optional[Dict] = None) -> Optional[Dict[str, Any]]:
        ...
        if not indicator_data or not isinstance(indicator_data, dict):
            self._log_indicator_trace(name_or_alias, None, status="FAILED", reason="Missing or invalid data object.")
            return None
        
        status = indicator_data.get("status", "").lower()
        if "error" in status or "failed" in status:
            self._log_indicator_trace(name_or_alias, status, status="FAILED", reason=f"Indicator reported status: {status}")
            return None
        
        self._log_indicator_trace(name_or_alias, "OK") # Log successful retrieval
        return indicator_data

    def _get_candlestick_confirmation(self, direction: str, min_reliability: str = 'Medium') -> Optional[Dict[str, Any]]:
        # This helper's logic remains largely unchanged but now benefits from safer get_indicator
    ...
    def _get_trend_confirmation(self, direction: str) -> bool:
        """ ✅ NEW: Generalized HTF confirmation engine. """
