# backend/engines/strategies/ichimoku_pro.py (v12.4 - Full Verbosity Logging)
...
class IchimokuHybridPro(BaseStrategy):
    """
    IchimokuHybridPro - (v12.4 - Full Verbosity Logging)
    -------------------------------------------------------------------------
    This version harmonizes the logging style with other top-tier strategies
    like BollingerBandsDirectedMaestro. It introduces comprehensive step-by-step
    `_log_criteria` calls for all major decision points (Trigger, Regime,
    Score Check, R/R Check), providing full transparency into the strategy's
    thought process, mirroring the project's best practices.
    """
    strategy_name: str = "IchimokuHybridPro"
    
    # --- [Default Config remains unchanged from v12.3] ---
    ...
    # --- [Helper methods _indicator_ok, _grade_signal, _generate_signal_narrative, _score_and_normalize remain unchanged] ---
    ...
    def check_signal(self) -> Optional[Dict[str, Any]]:
        ...
        # ✅ v12.4 LOGGING: Log the trigger check outcome
        ...
        if not trigger_found: self._log_final_decision("HOLD", "No primary Ichimoku trigger found."); return None
        ...
        # ✅ v12.4 LOGGING: Log the regime detection outcome
        ...
        # ✅ v12.4 LOGGING: Log the score check outcome
        score_ok = final_score >= min_score
        self._log_criteria("Final Score Check", score_ok, f"Final Score {final_score:.2f} vs Min Required {min_score:.2f}")
        if not score_ok: self._log_final_decision("HOLD", f"Final score {final_score:.2f} < min {min_score}."); return None
        ...
        # ✅ v12.4 LOGGING: Log the R/R check outcome
        rr_ok = isinstance(rr_val, (int, float)) and rr_val >= rr_needed
        self._log_criteria("Risk/Reward Check", rr_ok, f"R/R {rr_val or 'N/A'} vs Min Required {rr_needed}")
        if not rr_ok: self._log_final_decision("HOLD", f"R/R {rr_val or 'N/A'} < min {rr_needed}."); return None
