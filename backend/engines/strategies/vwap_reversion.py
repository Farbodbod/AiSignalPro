# backend/engines/strategies/vwap_reversion.py (v7.0 - The Transplant Edition)
...
class VwapMeanReversion(BaseStrategy):
    """
    VwapMeanReversion - (v7.0 - The Transplant Edition)
    ---------------------------------------------------------------------------
    This definitive version is the result of a complete architectural refactor
    ("transplant"). The unique trading logic of VwapMeanReversion has been
    transplanted into a proven, stable, and robust strategy structure,
    eradicating a persistent, elusive 'phantom bug'. It now features the
    "Sentinel" deep data check and high-fidelity ADX logging for maximum
    ...
    # ---------- Helpers (Preserved from original logic) ----------
    ...
    # ---------- Main Logic (Rebuilt on a stable chassis) ----------
    def check_signal(self) -> Optional[Dict[str, Any]]:
        ...
        # --- 1. The Sentinel: Dynamic & Deep Data Availability Check ---
        ...
        adx_raw_value = (indicators['adx'].get('values') or {}).get('adx')
        adx_strength = self._to_float(self._get_latest_value(adx_raw_value), default=None)

        if adx_strength is None:
            self._log_criteria("Market Regime (ADX)", False, "ADX value could not be parsed into a valid number.")
            self._log_final_decision("HOLD", "ADX data was not available for market regime check.")
            return None

        # ✅ HIGH-FIDELITY LOGGING: The ADX value is always reported.
        ...
        if not adx_ok:
            self._log_final_decision("HOLD", f"ADX filter failed. {regime_msg}")
            return None
        ...
        if entry_price is None or atr_value is None:
            self._log_final_decision("HOLD", "Risk data missing (entry/atr).")
            return None
        ...
        if signal_direction == "BUY":
            sl_source_band = self._to_float(self._get_latest_value(vwap_values.get('lower_band')))
            if sl_source_band is not None: stop_loss = sl_source_band - (atr_value * atr_mult)
        else: # SELL
            sl_source_band = self._to_float(self._get_latest_value(vwap_values.get('upper_band')))
            if sl_source_band is not None: stop_loss = sl_source_band + (atr_value * atr_mult)

        if stop_loss is None:
            self._log_final_decision("HOLD", "VWAP bands not available for SL calculation.")
            return None
