...
class OracleXPro(BaseStrategy):
    ...
    🚀 KEY EVOLUTIONS in v2.0:
    1.  **Full OHRE v3.0 Integration:** The custom SL logic and call to the legacy
        risk engine have been replaced by a clean, simple call to the new
        `_orchestrate_static_risk`, which now handles the entire exit strategy.
    2.  **Adaptive HTF Context:** The "Quantum Observatory" (Phase 1) has been
        upgraded to use ADX percentiles, making its strategic bias detection
        fully adaptive and more reliable.
    3.  **Preservation of Core Logic:** The strategy's unique and powerful PRZ
        detection and confirmation scoring engines are 100% preserved.
    """
    strategy_name: str = "OracleXPro"

    # --- Default config updated for new architecture ---
    default_config: ClassVar[Dict[str, Any]] = {
        # Phase 1: The Quantum Observatory (Upgraded)
        "htf_context_config": {
            "enabled": True,
            "min_adx_percentile": 75.0
        },
        # Phase 2: The Sniper's Hideout (Unchanged)
        "prz_config": {
            "use_fibonacci": True,
        ...
        # Phase 3: The Firing Protocol (Unchanged)
        ...
        # Phase 4 is now handled by OHRE v3.0
        
        "htf_confirmation_enabled": True,
        "htf_map": {"5m": "15m", "15m": "1h", "1h": "4h", "4h": "1d"},
    }

    # --- Helper methods _find_oracle_prz and _calculate_confirmation_score are 100% preserved ---
    ...
    def check_signal(self) -> Optional[Dict[str, Any]]:
        ...
        if htf_cfg.get('enabled', True) and self.htf_analysis:
            ...
            # UPGRADED: Using ADX percentile instead of fixed value
            htf_adx_percentile = self._safe_get(htf_adx, ['analysis', 'adx_percentile'], 0)
            
            htf_bias = "NEUTRAL"
            min_adx_pct = htf_cfg.get('min_adx_percentile', 75.0)
            if htf_trend_score >= 5 and self._is_valid_number(htf_adx_percentile) and htf_adx_percentile >= min_adx_pct:
                htf_bias = "BULLISH"; direction = "BUY"
            elif htf_trend_score <= -5 and self._is_valid_number(htf_adx_percentile) and htf_adx_percentile >= min_adx_pct:
                htf_bias = "BEARISH"; direction = "SELL"
            
            self._log_criteria("Phase 1: HTF Context (Adaptive)", htf_bias not in ["NEUTRAL", "NOT_CHECKED"], f"HTF Bias is '{htf_bias}'. Ichi Score: {htf_trend_score}, ADX %: {htf_adx_percentile or 'N/A':.2f}")
            if htf_bias == "NEUTRAL":
        ...
        # --- Data Availability for Primary Timeframe (Upgraded for OHRE) ---
        required = ['fibonacci', 'pivots', 'structure', 'rsi', 'stochastic', 'whales', 'patterns', 'atr', 'adx', 'ichimoku']
        ...
        # --- Phase 4: The Exit Strategy (✅ UPGRADED to OHRE v3.0) ---
