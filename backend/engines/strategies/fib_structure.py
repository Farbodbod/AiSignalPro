...
class ConfluenceSniper(BaseStrategy):
    ...
    default_config: ClassVar[Dict[str, Any]] = {
        ...
        "confluence_proximity_percent": 0.8,
    ...
    def _calculate_confluence_score(self, direction: str, rsi_data: Dict, stoch_data: Dict, whales_data: Dict) -> tuple[int, List[str]]:
        ...
        if rsi_confirm and stoch_confirm:
            ...
            confirmations.append("Dual Oscillator")
        elif rsi_confirm or stoch_confirm:
            score += weights.get('single_oscillator', 1)
            confirmations.append("Single Oscillator")
        ...
        if (whales_data.get('analysis') or {}).get('is_climactic_volume', False):
    ...
    def _get_oscillator_confirmation(self, direction: str, rsi_data: Dict, stoch_data: Dict) -> tuple[bool, bool]:
        ...
        if rsi_val is not None:
            ...
            if direction == "BUY" and rsi_val < rsi_os:
                rsi_confirm = True
            elif direction == "SELL" and rsi_val > rsi_ob:
                rsi_confirm = True
        if stoch_pos is not None:
            if direction == "BUY" and "Oversold" in stoch_pos:  # ✅ اصلاحیه ۱
                stoch_confirm = True
            elif direction == "SELL" and "Overbought" in stoch_pos:  # ✅ اصلاحیه ۱
                stoch_confirm = True
    ...
    def _find_best_prz(self, cfg: Dict, fib_data: Dict, structure_data: Dict) -> Tuple[Optional[Dict], Dict]:
        ...
        fib_levels = {
            lvl['level']: lvl['price']
            for lvl in fib_data.get('values', {}).get('levels', [])
            if 'Retracement' in lvl.get('type', '')
        }
    ...
    def check_signal(self) -> Optional[Dict[str, Any]]:
        ...
        if not direction:
            self._log_final_decision("HOLD", "No primary swing trend identified by Fibonacci.")
            return None
        ...
        if not self._is_valid_number(entry_price, atr_value, structure_level):  # ✅ اصلاحیه ۲
            self._log_final_decision("HOLD", "Risk data missing for SL calculation.")
            return None
