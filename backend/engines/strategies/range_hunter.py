# backend/engines/strategies/range_hunter.py (v3.0 - The Quantum Oscillator)
...
class RangeHunterPro(BaseStrategy):
    """
    RangeHunterPro - (v3.0 - The Quantum Oscillator)
    -------------------------------------------------------------------------
    This version represents a complete evolution based on the Quantum Roadmap,
    transforming the strategy from a conflicted hunter into a precise and
    pragmatic Quantum Oscillator.

    🚀 KEY EVOLUTIONS in v3.0:
    1.  **Redefined Battlefield:** The regime filter is now adaptive, using ADX
        percentiles and a more realistic BBW quantile to identify more opportunities.
    2.  **Evolved Trigger Squad:** The fragile single-indicator trigger has been
        replaced with a robust, weighted scoring engine that seeks confluence
        from RSI, Stochastic, and Candlestick patterns.
    3.  **Harmonized Arsenal:** All custom risk logic is removed. The strategy is
        now fully integrated with the OHRE v3.0 engine, using an override for
        its unique R/R profile to maximize its high-probability setups.
    """
    strategy_name: str = "RangeHunterPro"

    default_config: ClassVar[Dict[str, Any]] = {
        # --- G.1: Upgraded Regime Filters ---
        "regime_filter": {
            "enabled": True,
    ...
    def check_signal(self) -> Optional[Dict[str, Any]]:
        ...
        if regime_cfg.get('enabled'):
            ...
            if regime_cfg.get('bbw_squeeze_enabled'):
                bbw_quantile_val = self._safe_get(indicators, ['bollinger', 'analysis', 'bbw_quantile_value'], 1.0)
                if bbw_quantile_val > regime_cfg.get('bbw_quantile', 0.40):
                    self._log_final_decision("HOLD", "Market volatility is too high (BBW Quantile too high).")
                    return None
                self._log_criteria("Bollinger Squeeze Filter", True, f"Market is in low-volatility (BBW Quantile: {bbw_quantile_val:.2f})")
        ...
        # Set the strategy-specific R/R override for the OHRE engine
        ...
        # --- 5. Final Decision ---
        confirmations = {
            "market_regime": "RANGING (ADX/BBW Confirmed)",
            "final_score": score,
