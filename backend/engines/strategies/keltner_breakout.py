# backend/engines/strategies/keltner_breakout.py (v9.3 - The Self-Contained Engine)
...
class KeltnerMomentumBreakout(BaseStrategy):
    """
    KeltnerMomentumBreakout - (v9.3 - The Self-Contained Engine)
    -------------------------------------------------------------------------
    This is the definitive, hedge-fund-grade version. It contains the final
    architectural patch to make its default_config fully "self-contained"
    by including default htf_map and htf_confirmations, ensuring robust
    fallback behavior and maximum system stability.
    """
    ...
    default_config: ClassVar[Dict[str, Any]] = {
        ...
        # ✅ ARCHITECTURAL HOTFIX (v9.3): Add default HTF configs to make the module self-contained.
    ...
    def _calculate_momentum_score(self, direction: str, indicators: Dict) -> Tuple[int, List[str]]:
        ...
        # This now correctly uses the self-contained defaults if not present in config.json
    ...
    def _get_min_score_for_tf(self) -> int:
        # ... [This method is unchanged and correct] ...
        cfg = self.config; min_cfg = cfg.get('min_momentum_score', {"low_tf": 7, "high_tf": 10})
        tf = getattr(self, "primary_timeframe", "15m")
        return int(min_cfg.get('low_tf', 7)) if tf in ('1m','3m','5m','15m') else int(min_cfg.get('high_tf', 10))
