# backend/engines/strategies/keltner_breakout.py (v5.0 - The Sentinel Edition)
...
class KeltnerMomentumBreakout(BaseStrategy):
    """
    KeltnerMomentumBreakout - (v5.0 - The Sentinel Edition)
    -------------------------------------------------------------------------
    This major upgrade integrates two new sentinel layers from the BaseStrategy
    framework: The Outlier Candle Shield and the Market Regime Filter. This
    elevates the strategy's intelligence by ensuring it only operates on valid
    data and in suitable (trending) market conditions, significantly improving
    signal quality and robustness. All original logic is preserved.
    """
    ...
    def check_signal(self) -> Optional[Dict[str, Any]]:
        ...
        if cfg.get('outlier_candle_shield_enabled'):
            ...
            if self._is_outlier_candle(atr_multiplier=atr_multiplier):
                self._log_final_decision("HOLD", "Outlier candle detected, signal suppressed for safety.")
                return None
        ...
        if cfg.get('market_regime_filter_enabled'):
            ...
            if not regime_is_ok:
                self._log_final_decision("HOLD", "Market regime is not suitable for this strategy.")
                return None
        ...
        # --- 5. Qualification (Momentum Score) ---
