...
class TrendRiderStrategy(BaseStrategy):
    ...
    def check_signal(self) -> Optional[Dict[str, Any]]:
        supertrend_data = self.analysis.get('supertrend'); adx_data = self.analysis.get('adx')
        ...
        if supertrend_data['signal'] == "Bullish Trend Change":
            ...
            if htf_supertrend_data and htf_supertrend_data.get('trend') == "Uptrend": htf_trend_aligned = True
        elif supertrend_data['signal'] == "Bearish Trend Change":
            signal_direction = "SELL"
            if htf_supertrend_data and htf_supertrend_data.get('trend') == "Downtrend": htf_trend_aligned = True
        ...
        confirmations = {"supertrend_signal": supertrend_data['signal'], "adx_strength": round(adx_data['adx'], 2), "htf_confirmation": "Aligned"}
        return {"strategy_name": self.strategy_name, "direction": signal_direction, "entry_price": entry_price, **risk_params, "confirmations": confirmations}
