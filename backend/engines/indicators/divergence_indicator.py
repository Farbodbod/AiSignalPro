# backend/engines/indicators/divergence_indicator.py (v5.1 - Deployment Test Version)
import pandas as pd
import logging
...
class DivergenceIndicator(BaseIndicator):
    """
    Divergence Engine - (v5.1 - Deployment Test Version)
    -----------------------------------------------------------------------------------
    This version includes a critical log message to definitively test if the new
    file is being executed in the deployment environment.
    """
    ...
    def calculate(self) -> 'DivergenceIndicator':
        # ✅✅✅ --- TEST LOG --- ✅✅✅
        logger.critical(f"✅✅✅ EXECUTING DIVERGENCE INDICATOR v5.1 ON {self.timeframe} - NEW ARCHITECTURE IS LIVE! ✅✅✅")
        # ✅✅✅ --- END OF TEST LOG --- ✅✅✅
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        # ... (بقیه کد analyze بدون تغییر باقی می‌ماند) ...
        ...
        for i in range(len(previous_pivots)):
            ...
            if prev_pivot[self.pivots_col] == 1 and last_pivot[self.pivots_col] == 1:
            ...
            elif prev_pivot[self.pivots_col] == -1 and last_pivot[self.pivots_col] == -1:
            ...
            if divergence:
                signals.append({
                    **divergence,
                    "pivots": [
                        {"time": prev_pivot.name.strftime('%Y-%m-%d %H:%M:%S'), "price": round(price1, 5), "oscillator_value": round(rsiname, 2)},
