# engines/strategy_engine.py (نسخه 12.0 - بهینه‌سازی حرفه‌ای ریسک و ساختاردهی)
...
class StrategyEngine:
    ...
    def _calculate_targets(self, direction: str, entry_price: float, stop_loss: float, custom_target: Optional[float] = None) -> List[float]:
        ...
        if direction == 'BUY':
            targets += [
                round(entry_price + (risk_amount * 1.5), 4),
                round(entry_price + (risk_amount * 2.5), 4)
            ]
        elif direction == 'SELL':
            targets += [
                round(entry_price - (risk_amount * 1.5), 4),
    ...
    def _generate_pivot_strategy(self, direction: str) -> Dict[str, Any]:
        if not self.entry_price or not self.atr:
            return {}
        ...
        if direction == 'BUY':
            ...
            if relevant:
                stop_loss = round(max(relevant) - (self.atr * 0.1), 4)
        elif direction == 'SELL':
            relevant = [p[1] for p in pivots if p[1] > self.entry_price]
            if relevant:
                stop_loss = round(min(relevant) + (self.atr * 0.1), 4)
        ...
        return {
            ...
            "stop_loss": stop_loss,
            "targets": targets,
            "strategy_name": "Pivot Reversion",
            "meta": {"direction": direction, "source": "pivot"}
        }
    ...
    def _generate_trend_strategy(self, direction: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "stop_loss": stop_loss,
            "targets": targets,
            "strategy_name": "Trend Hunter",
            "meta": {"direction": direction, "source": "trend"}
        }
    ...
    def _generate_volatility_breakout_strategy(self, direction: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "meta": {"direction": direction, "source": "volatility"}
        }

    def _generate_range_strategy(self) -> Dict[str, Any]:
        pivots = self.market_structure.get("pivots", [])
        if len(pivots) < 4 or not self.entry_price or not self.atr:
            return {}
        ...
        if abs(self.entry_price - support) < (self.atr * 0.3):
            direction = "BUY"
            stop_loss = round(support - (self.atr * 0.3), 4)
            ...
            direction = "SELL"
            stop_loss = round(resistance + (self.atr * 0.3), 4)
        ...
        return {
            ...
            "stop_loss": stop_loss,
            "targets": targets,
            "strategy_name": "Range Hunter",
            "meta": {"direction": direction, "source": "range"}
        }
    ...
    def generate_strategy(self, signal_type: str) -> Dict[str, Any]:
        ...
        if is_breakout or is_volatility_spike:
            logger.info("Volatility/Breakout detected.")
            strategy = self._generate_volatility_breakout_strategy(signal_type)
        elif "Strong" in trend_signal:
            logger.info(f"Strong trend detected: {trend_signal}")
            strategy = self._generate_trend_strategy(signal_type)
        elif "Ranging" in trend_signal:
            logger.info("Ranging market detected.")
            strategy = self._generate_range_strategy()
        else:
            logger.info(f"Weak/Uncertain market: {trend_signal}")
            strategy = self._generate_pivot_strategy(signal_type)
