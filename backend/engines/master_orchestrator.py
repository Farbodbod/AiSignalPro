# engines/master_orchestrator.py (v29.0 - Fully Async & Hardened)
...
class MasterOrchestrator:
    """
    The strategic mastermind of AiSignalPro (v29.0 - Fully Async & Hardened).
    -------------------------------------------------------------------------
    This version fully integrates with the async IndicatorAnalyzer, awaits its
    coroutines, and preserves all advanced features including the full Gemini
    prompt, HTF Quality Gate, and Focus Mode compatibility.
    """
    def __init__(self, config: Dict[str, Any]):
        ...
        self.ENGINE_VERSION = "29.0.0"
        
        logger.info(f"MasterOrchestrator v{self.ENGINE_VERSION} (Fully Async) initialized.")
    ...
    async def run_analysis_pipeline(self, df: pd.DataFrame, symbol: str, timeframe: str, previous_df: Optional[pd.DataFrame] = None) -> Tuple[Optional[Dict[str, Any]], Optional[pd.DataFrame]]:
        try:
            ...
            # ✅ CRITICAL FIX: Await the async methods from the new IndicatorAnalyzer
...
"""
        ...
        # Assuming gemini_handler is updated to have an async method
        ...
        if ai_response.get('signal') == 'HOLD':
            logger.warning(f"AI VETOED for {symbol}. Reason: {ai_response.get('explanation_fa')}")
            return None 
