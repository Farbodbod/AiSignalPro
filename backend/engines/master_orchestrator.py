# engines/master_orchestrator.py (v29.1 - World-Class Peer-Reviewed Edition)
...
class MasterOrchestrator:
    """
    The strategic mastermind of AiSignalPro (v29.1 - World-Class Peer-Reviewed).
    -------------------------------------------------------------------------
    This version is the result of an expert peer review, hardening all identified
    edge cases related to AI validation, HTF data handling, cooldown logic, and
    error handling to make it a truly enterprise-grade orchestration engine.
    All core logic and the full Gemini prompt are 100% preserved.
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # FIX 3: Key is now (symbol, timeframe)
        self.last_gemini_call_times: Dict[Tuple[str, str], float] = {}
        self.ENGINE_VERSION = "29.1.0"
    ...
    async def run_analysis_pipeline(
        ...
        try:
            ...
            # FIX 5: Logging is the primary notification. The 'None' return is handled by the worker.
            logger.error(
    ...
    async def run_strategy_pipeline(
        ...
        """
        Runs all strategies, applies HTF confirmation, filters signals,
        and passes candidate to Gemini AI for final validation.
        """
        valid_signals = []
        strategies_config = self.config.get("strategies", {})
        ...
        for sc in self._strategy_classes:
            ...
            try:
                ...
                if merged_strat_config.get("htf_confirmation_enabled"):
                    ...
                    if target_htf and target_htf != timeframe:
                        if target_htf in htf_context:
                            ...
                            # FIX 2: Hardened check for DataFrame type.
                ...
                if signal:
                    ...
                    # FIX 4: Add logging for missing R/R for easier debugging.
                    if "risk_reward_ratio" not in signal:
                        logger.debug(
                            f"Strategy '{strategy_name}' produced a signal without 'risk_reward_ratio'. Defaulting to 0."
                        )
        ...
        if not best_signal:
            ...
            # FIX 6: Safe sorting that prevents ValueError if a strategy is not in the priority list.
            qualified_signals.sort(
                key=lambda s: priority_list.index(s["strategy_name"]) if s["strategy_name"] in priority_list else 99
            )
        ...
        if ai_confirmation is None:
            return {
                "status": "NEUTRAL",
                "message": "Signal was vetoed by AI or AI response was invalid.",
                "full_analysis": primary_analysis,
                "engine_version": self.ENGINE_VERSION,
            }
    ...
    def _find_super_signal(self, signals: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """
        Detects SuperSignal confluence based on strategy agreement.
        """
    ...
    async def _get_ai_confirmation(self, signal: Dict[str, Any], symbol: str, timeframe: str) -> Optional[Dict[str, Any]]:
        """
        Calls Gemini AI for final signal validation with strict schema enforcement.
        """
        # FIX 3: Cooldown is now per symbol AND timeframe.
        ...
        if (time.time() - last_call_time) < cooldown:
            ...
            return {
                ...
                "explanation_fa": "AI analysis skipped due to per-symbol cooldown.",
            }
        ...
        # ✅ The full, un-summarized prompt is 100% preserved.
        prompt_template = f"""
...
"""
        ...
        # FIX 7: Future-proof check for async Gemini client.
        ...
        # FIX 1: Hardened AI response validation.
        try:
            if not isinstance(ai_response, dict):
                raise TypeError("Response is not a dictionary.")

            validated_signal = str(ai_response["signal"])
            if validated_signal.upper() not in ["BUY", "SELL", "HOLD"]:
                raise ValueError(f"Invalid signal value: {validated_signal}")

            validated_confidence = int(ai_response["confidence_percent"])
            if not (0 <= validated_confidence <= 100):
                raise ValueError(f"Confidence out of range: {validated_confidence}")

            if "explanation_fa" not in ai_response:
                raise KeyError("Missing 'explanation_fa' key.")

        except (TypeError, ValueError, KeyError, AttributeError) as e:
            logger.critical(
                f"FATAL: AI response schema validation failed for {symbol}@{timeframe}. Error: {e}. Response: {ai_response}"
            )
            return None
