...
from .indicator_analyzer import calculate_indicators from .trend_analyzer import analyze_trend from .market_structure_analyzer import LegPivotAnalyzer from .strategy_engine import StrategyEngine from .candlestick_reader import CandlestickPatternDetector from .divergence_detector import detect_divergences from .whale_analyzer import WhaleAnalyzer from .gemini_handler import GeminiHandler

logger = logging.getLogger(name)

ENGINE_VERSION = "7.0.0" TIMEFRAME_WEIGHTS = {'1d': 3, '4h': 2.5, '1h': 2, '15m': 1, '5m': 0.5} SCORE_THRESHOLD = 5.0 GEMINI_CALL_COOLDOWN_SECONDS = 300

class MasterOrchestrator: def init(self): self.gemini_handler = GeminiHandler() self.last_gemini_call_time = 0 logger.info(f"MasterOrchestrator initialized. Engine Version: {ENGINE_VERSION}")
...
def analyze_single_dataframe(self, df: pd.DataFrame, timeframe: str, symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        raw_analysis["indicators"] = indicators
...
async def get_multi_timeframe_signal(self, all_tf_analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for tf, data in all_tf_analysis.items():
        ...
        if activity == 'volume_spike':
            buy_score += 0.75 * weight
        elif activity == 'anomaly':
    ...
    gemini_confirmation = {
        "signal": "N/A",
        "confidence": 0,
        "explanation_fa": "AI analysis not triggered."
    }
    ...
    if final_signal != "HOLD":
        ...
        if (now - self.last_gemini_call_time) < GEMINI_CALL_COOLDOWN_SECONDS:
            ...
            prompt_details = {
                tf: {k: v for k, v in data.items() if k != 'dataframe'}
                for tf, data in all_tf_analysis.items()
            }
            prompt = (
                f'Analyze this JSON data for {symbol} and provide a response ONLY in JSON format with keys '
                f'"signal" (BUY/SELL/HOLD) and "explanation_fa" (concise, professional explanation in Persian).\n'
                f'Technical Data: {json.dumps(prompt_details, indent=2, default=str)}'
            )
