...
class MasterOrchestrator:
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.ENGINE_VERSION = "18.0.0"
        logger.info(f"MasterOrchestrator v{self.ENGINE_VERSION} (MTF-Aware) initialized.")
    
    def _get_default_config(self) -> Dict[str, Any]:
        return {"gemini_cooldown_seconds": 300, "min_confluence_for_super_signal": 3, "strategy_priority": ["SuperSignal Confluence", "DivergenceSniper", "VolumeCatalyst", "IchimokuPro", "TrendRider", "PivotReversalStrategy", "MeanReversionStrategy"], 'TrendRiderStrategy': {'min_adx_strength': 25}, 'MeanReversionStrategy': {'rsi_oversold': 30, 'rsi_overbought': 70}, 'DivergenceSniperStrategy': {'bullish_reversal_patterns': ['HAMMER', 'MORNINGSTAR', 'BULLISHENGULFING'], 'bearish_reversal_patterns': ['SHOOTINGSTAR', 'EVENINGSTAR', 'BEARISHENGULFING']}, 'PivotReversalStrategy': {'proximity_percent': 0.002, 'stoch_oversold': 25, 'stoch_overbought': 75},'VolumeCatalystStrategy': {'enabled': True},'IchimokuProStrategy': {'min_score_to_signal': 5}}
    ...
    def run_full_pipeline(self, df_ltf: pd.DataFrame, timeframe_ltf: str, df_htf: pd.DataFrame, timeframe_htf: str, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        for strategy_class in self._strategy_classes:
            ...
            if signal: valid_signals.append(signal)
        ...
        return { "symbol": symbol, "timeframe": timeframe_ltf, "base_signal": best_signal, "ai_confirmation": ai_confirmation, "full_analysis": analysis_ltf }
