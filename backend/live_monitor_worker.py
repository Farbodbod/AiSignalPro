# live_monitor_worker.py (v5.1 - Log Consolidation Edition)
import asyncio
import logging
import os
import django
...
# --- The rest of the initial setup code (imports, classes, etc.) remains unchanged ---
...
class SignalCache: # Unchanged
    def __init__(self, ttl_map_hours: Dict[str, int], default_ttl_hours: int):
        self._cache: Dict[Tuple[str, str, str], float] = {}; self.ttl_map_seconds = {tf: hours * 3600 for tf, hours in ttl_map_hours.items()}; self.default_ttl_seconds = default_ttl_hours * 3600
    def is_duplicate(self, symbol: str, timeframe: str, direction: str) -> bool:
        ...
        if key in self._cache and (time.time() - self._cache[key]) < ttl:
            remaining = ((self._cache[key] + ttl) - time.time()) / 60; logger.info("Duplicate signal found", key=key, cooldown_minutes_left=f"{remaining:.1f}"); return True
        return False
    def store_signal(self, symbol: str, timeframe: str, direction: str):
        key = (symbol, timeframe, direction); self._cache[key] = time.time(); logger.info("Signal stored in cache", key=key)

@sync_to_async # Unchanged
def save_analysis_snapshot(symbol: str, timeframe: str, package: Dict[str, Any]):
    try:
        status, sanitized_package = package.get("status", "NEUTRAL"), convert_numpy_types(package); full_analysis = sanitized_package.get("full_analysis", {}); signal_data = sanitized_package if status == "SUCCESS" else None
...
async def run_single_analysis(symbol: str, timeframe: str, orchestrator: MasterOrchestrator, fetcher: ExchangeFetcher, analysis_state: Dict, global_context: Dict, new_states: Dict, semaphore: asyncio.Semaphore) -> Tuple[str, str, str]: # Unchanged
    task_id = f"{symbol}@{timeframe}"
    async with semaphore:
        try:
            logger.debug("Starting analysis task", task_id=task_id); state_key, previous_df = (symbol, timeframe), analysis_state.get((symbol, timeframe)); kline_limit = orchestrator.config.get("general", {}).get("fetcher_limit", 500)
            df, source = await fetcher.get_first_successful_klines(symbol, timeframe, limit=kline_limit)
            if df is None or df.empty: logger.warning("Could not fetch kline data.", task_id=task_id, source=source); return ("warning", task_id, "Missing kline data")
            ...
            logger.debug("Analysis task complete", task_id=task_id); return ("success", task_id, "OK")
        except Exception as e:
            logger.error("Error in analysis phase", task_id=task_id, error=str(e), exc_info=True); return ("error", task_id, str(e))

async def run_single_strategy(symbol: str, timeframe: str, orchestrator: MasterOrchestrator, global_context: Dict, cache: SignalCache, telegram: TelegramHandler, semaphore: asyncio.Semaphore) -> Tuple[str, Optional[Dict]]: # Unchanged
    task_id = f"{symbol}@{timeframe}"
    async with semaphore:
        try:
            ...
            if final_signal_package and final_signal_package.get("status") == "SUCCESS":
                ...
                if direction and not cache.is_duplicate(symbol, timeframe, direction):
                    adapter, message = SignalAdapter(signal_package=final_signal_package), adapter.to_telegram_message(); logger.info(f"🚀🚀 SIGNAL DETECTED! Preparing alert for {symbol}@{timeframe} {direction}")
                    success = await telegram.send_message_async(message)
                    if success: cache.store_signal(symbol, timeframe, direction); return ("signal_sent", {"symbol": symbol, "timeframe": timeframe, "direction": direction})
                    else: return ("send_failed", {"symbol": symbol, "timeframe": timeframe, "direction": direction})
            return ("no_signal", None)
        except Exception as e:
            logger.error("Error in strategy phase", task_id=task_id, error=str(e), exc_info=True); return ("error", None)

def setup_logging(log_level_str: str): # Unchanged
    log_level = getattr(logging, log_level_str.upper(), logging.INFO)
    shared_processors = [structlog.contextvars.merge_contextvars, structlog.stdlib.add_logger_name, structlog.stdlib.add_log_level, structlog.processors.TimeStamper(fmt="iso"),]
    structlog.configure(processors=shared_processors + [structlog.stdlib.PositionalArgumentsFormatter(), structlog.processors.StackInfoRenderer(), structlog.processors.format_exc_info, structlog.stdlib.ProcessorFormatter.wrap_for_formatter,], logger_factory=structlog.stdlib.LoggerFactory(), wrapper_class=structlog.stdlib.BoundLogger, cache_logger_on_first_use=True,)
    formatter = structlog.stdlib.ProcessorFormatter(processor=structlog.dev.ConsoleRenderer(colors=True),)
    handler = RichHandler(level=log_level, rich_tracebacks=True); handler.setFormatter(formatter)
    root_logger = logging.getLogger(); root_logger.addHandler(handler); root_logger.setLevel(log_level)
    logging.getLogger("httpx").setLevel(logging.WARNING); logging.getLogger("django").setLevel(logging.WARNING)
