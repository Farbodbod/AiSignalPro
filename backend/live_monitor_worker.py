# live_monitor_worker.py (v4.0 - Fully Async & Hardened)
...
async def run_single_analysis(
    ...
    """Helper function for Phase 1 to ensure clean async execution."""
    async with semaphore:
        try:
            ...
            analysis_result, new_state_df = await orchestrator.run_analysis_pipeline(
                df, symbol, timeframe, previous_df=previous_df
            )
            ...
            logger.error(f"Error in analysis phase for {symbol}@{timeframe}: {e}", exc_info=True)
...
async def run_single_strategy(
    ...
    """Helper function for Phase 2 to ensure clean async execution."""
    async with semaphore:
        try:
            ...
            # ✅ FIX: Await the async pipeline
            final_signal_package = await orchestrator.run_strategy_pipeline(
                primary_analysis, htf_context, symbol, timeframe
            )
            ...
            if final_signal_package and final_signal_package.get("status") == "SUCCESS":
                ...
                if direction and not cache.is_duplicate(symbol, timeframe, direction):
                    ...
                    logger.info(
                        f"🚀🚀 SIGNAL DETECTED! Preparing alert for {symbol}@{timeframe} {direction}"
                    )
            ...
            logger.error(f"Error in strategy phase for {symbol}@{timeframe}: {e}", exc_info=True)
...
async def main_loop():
    ...
    logging.getLogger("core").setLevel(root_log_level)
    logging.getLogger("engines").setLevel(root_log_level)
    ...
    logger.info("=" * 50)
    logger.info(f"  AiSignalPro Live Worker (v{version}) - Fully Async Architecture")
    logger.info(f"  Root Log Level set to: {logging.getLevelName(root_log_level)}")
    logger.info(f"  Monitoring {len(symbols)} symbols on {len(timeframes)} timeframes.")
    logger.info("=" * 50)

    await telegram.send_message_async(
        f"✅ *AiSignalPro Bot (v{version}) is LIVE!* (Log Level: {log_level_str})"
    )
    ...
    while True:
        ...
        global_context = {s: {} for s in symbols}
        new_states: Dict = {}
        ...
        analysis_tasks = [
            run_single_analysis(
                s, tf, orchestrator, fetcher, analysis_state, global_context, new_states, semaphore
            )
            for s in symbols
        ...
        strategy_tasks = [
            run_single_strategy(s, tf, orchestrator, global_context, cache, telegram, semaphore)
            for s in symbols
            for tf in timeframes
        ]
        ...
        logger.info(
            f"--- Cycle #{cycle_count} finished in {cycle_duration:.2f} seconds. "
