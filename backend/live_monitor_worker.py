...
class SignalCache:
    ...
    def store_signal(self, symbol: str, timeframe: str, direction: str):
        self._cache[(symbol, timeframe, direction)] = time.time(); logger.info(f"Signal {(symbol, timeframe, direction)} stored in cache.")

@sync_to_async
def save_analysis_snapshot(symbol: str, timeframe: str, package: Dict[str, Any]):
    # ... (کد این تابع بدون تغییر است)
    try:
        status, sanitized_package = package.get("status", "NEUTRAL"), convert_numpy_types(package)
...
async def main_loop():
    ...
    logger.info("="*50); logger.info(f"  AiSignalPro Live Worker (v{version}) - Fully Async Architecture")
    logger.info(f"  Root Log Level set to: {logging.getLevelName(root_log_level)}")
    logger.info(f"  Monitoring {len(symbols)} symbols on {len(timeframes)} timeframes."); logger.info("="*50)
