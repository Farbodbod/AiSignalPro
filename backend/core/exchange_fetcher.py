# core/exchange_fetcher.py (نسخه افسانه‌ای و بی‌نقص v1.1)
...
EXCHANGE_CONFIG = {
    'mexc': {'base_url': 'https://api.mexc.com', 'kline_endpoint': '/api/v3/klines', 'symbol_template': '{base}{quote}', 'timeframe_map': {'5m': '5m', '10m': '10m', '15m': '15m', '1h': '1h', '4h': '4h', '1d': '1d'}, 'rate_limit_delay': 1.0},
    'kucoin': {'base_url': 'https://api.kucoin.com', 'kline_endpoint': '/api/v1/market/candles', 'symbol_template': '{base}-{quote}', 'timeframe_map': {'5m': '5min', '10m': '15min', '15m': '15min', '1h': '1hour', '4h': '4hour', '1d': '1day'}, 'rate_limit_delay': 1.0},
    'okx': {'base_url': 'https://www.okx.com', 'kline_endpoint': '/api/v5/market/candles', 'symbol_template': '{base}-{quote}', 'timeframe_map': {'5m': '5m', '10m': '15m', '15m': '15m', '1h': '1H', '4h': '4H', '1d': '1D'}, 'rate_limit_delay': 1.0},
}
...
class ExchangeFetcher:
    def __init__(self, cache_ttl: int = 60):
        headers = {'User-Agent': 'AiSignalPro/1.3.0', 'Accept': 'application/json'}
        self.client = httpx.AsyncClient(headers=headers, timeout=20, follow_redirects=True)
        self.cache = {}
        self.cache_ttl = cache_ttl
        logging.info("ExchangeFetcher (Legendary Edition v1.3) initialized.")
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=1, max=5), retry=retry_if_exception_type((httpx.RequestError, httpx.HTTPStatusError)), reraise=True)
    ...
    def _normalize_kline_data(self, data: List[list], source: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logging.error(f"Data normalization failed for {source}: {e}")
            return []
    ...
    async def get_klines_from_one_exchange(self, exchange: str, symbol: str, timeframe: str, limit: int = 200) -> Optional[List[Dict]]:
        ...
        try:
            ...
            logging.warning(f"Failed to fetch from {exchange} for {symbol}@{timeframe}. Reason: {e}")
        return None
