# core/exchange_fetcher.py (نسخه 1.8 با قابلیت Ticker)
...
class ExchangeFetcher:
    def __init__(self, cache_ttl: int = 60):
        headers = {'User-Agent': 'AiSignalPro/1.8.0', 'Accept': 'application/json'}
        self.client = httpx.AsyncClient(headers=headers, timeout=20, follow_redirects=True)
        self.cache = {}
        self.cache_ttl = cache_ttl
        logging.info("ExchangeFetcher (Legendary Edition v1.8) initialized.")
    ...
    async def get_ticker_from_one_exchange(self, exchange: str, symbol: str) -> Optional[Dict]:
        ...
        if raw_data:
            ...
            try:
                ...
                elif exchange == 'okx' and raw_data.get('data'): data = raw_data['data'][0]; price = float(data.get('last', 0)); change = float(data.get('chg24h', 0))
