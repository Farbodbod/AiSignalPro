# core/views.py (هماهنگ شده با SignalAdapter جدید)
...
async def system_status_view(request):
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        ...
        for task in asyncio.as_completed(tasks):
            name = tasks[task];
            try:
                res = await task; latency = round(res.elapsed.total_seconds() * 1000, 1); status = 'online' if 200 <= res.status_code < 400 else 'offline'; results.append({'name': name, 'status': status, 'ping': f"{latency}ms"})
            except Exception:
...
async def market_overview_view(request):
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            cg_task = asyncio.create_task(client.get("https://api.coingecko.com/api/v3/global")); fng_task = asyncio.create_task(client.get("https://api.alternative.me/fng/?limit=1")); cg_res = await cg_task;
            if cg_res.status_code == 200:
                data = cg_res.json().get('data', {}); response_data.update({'market_cap': data.get('total_market_cap', {}).get('usd', 0), 'volume_24h': data.get('total_volume', {}).get('usd', 0), 'btc_dominance': data.get('market_cap_percentage', {}).get('btc', 0)})
