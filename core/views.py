...
from .exchange_fetcher import fetch_all_tickers_concurrently, SafeRequest # نام تابع اصلاح شد
import time
...
def market_overview_view(request):
    ...
    try:
        coingecko_url = "https://api.coingecko.com/api/v3/global"; global_data = SafeRequest.get(coingecko_url)
        if global_data and 'data' in global_data:
            cg_data = global_data['data']; market_cap = cg_data.get('total_market_cap', {}).get('usd', 0); volume_24h = cg_data.get('total_volume', {}).get('usd', 0); btc_dominance = cg_data.get('market_cap_percentage', {}).get('btc', 0)
            response_data.update({'market_cap': f"${market_cap:,.0f}", 'volume_24h': f"${volume_24h:,.0f}", 'btc_dominance': f"{btc_dominance:.1f}%"})
    except Exception: pass
    try:
        fng_url = "https://api.alternative.me/fng/?limit=1"; fng_data = SafeRequest.get(fng_url)
...
def all_data_view(request):
    try:
        ...
        all_ticker_data = fetch_all_tickers_concurrently(target_sources, symbol_map) # نام تابع اصلاح شد
